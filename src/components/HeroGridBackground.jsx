...
export default function HeroGridBackground() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Ultra-faded base pattern (barely visible, faded background)
      const baseLineColor = isDark ? 'rgba(255, 255, 255, 0.012)' : 'rgba(0, 0, 0, 0.012)';
      const crosshairFadedColor = isDark ? 'rgba(255, 255, 255, 0.03)' : 'rgba(0, 0, 0, 0.03)';

      const mouseX = mouseRef.current.x;
      const mouseY = mouseRef.current.y;

      // 1. Draw Ultra-Faded Base Grid Lines (Barely perceptible across the rest of the hero)
      ctx.lineWidth = 1;
      ...
      // 2. Draw Crisp Highlighted Grid Lines ONLY in the Spotlight Area (Clean laser lines, no blurry fog)
      ...
      // 3. Draw Clean Crosshair '+' Marks (Faded everywhere, crisp & highlighted near cursor)
      for (let x = 0; x <= width; x += GRID_SIZE) {
        for (let y = 0; y <= height; y += GRID_SIZE) {
